...
class TestPoseMatchSegmentation(unittest.TestCase):
    ...
    def test_seg_data(self):
        print(
            self.seg_data.shape,
            self.longterm_seg_sorting.shape,
            self.instance_seg_sorting.shape,
            self.track_sorting.shape,
        )
        ...
        if False:
            ...
            for frame in range(self.seg_data.shape[0]):
                seg_data_tmp[frame, ...] = self.seg_data[
                    frame, self.longterm_seg_sorting[frame], ...
                ]
            ...
            print(
                "-" * 10,
                self.seg_data[frame, :, 0, 0, :],
                self.longterm_seg_sorting[frame],
                sep="\n",
            )
        ...
        if False:
            seg_data_tmp[
                np.where(extended_id_mask == 0)[0],
                self.instance_seg_sorting[extended_id_mask == 0] - 1,
                :,
                :,
            ] = self.seg_data[extended_id_mask == 0, :, :]
            print(np.unique(seg_data_tmp))

        if False:
            points_tmp = np.zeros_like(self.all_points)
            points_tmp[
                np.where(self.id_mask == 0)[0],
                self.embed_sorting[self.id_mask == 0] - 1,
                :,
                :,
            ] = self.all_points[self.id_mask == 0, :, :]
