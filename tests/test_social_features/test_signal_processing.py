...
class TestSignalProcessing(unittest.TestCase):
    ...
    def test_validate_against_brians_code(self):
        ...
        points_for_individual_from_pose_estimation, _ = self._poses.get_identity_poses(
        ...
        points_for_individual = np.zeros(
            np.delete(np.shape(self._points), 1), dtype=self._points.dtype
        )
        idxs = np.where(self._raw_embed_data == animal_idx)
        points_for_individual[idxs[0], :, :] = self._points[idxs[0], idxs[1], :, :]

        pointSpeeds = point_speeds.PointSpeeds(self._poses, self._poses.cm_per_pixel)
        per_frame = pointSpeeds.per_frame(animal_idx)

        nose_speeds = np.gradient(points_for_individual[:, 0, :], axis=0)
        nose_speeds = (
            np.hypot(nose_speeds[:, 0], nose_speeds[:, 1])
            * self._poses.cm_per_pixel
            * self._poses.fps
        )
        ...
        signalProcessingFeatures = pointSpeeds.signal_processing(
            identity=animal_idx, window_size=window_size, per_frame_values=nose_speeds
        )

        print("Signal Processing".ljust(30), "|", Color.BLUE + "Test Case" + Color.ENDC)
        for key in TEST_CASE:
            self.assertEqual(TEST_CASE[key], signalProcessingFeatures[key][frameIdx][0])
            print(
                f"{key}: {signalProcessingFeatures[key][frameIdx][0]}".ljust(30),
                "|",
