...
- **File→Export Video with Overlays:** Export a copy of the current video with the JABS overlays drawn on every frame, at the original video resolution. A dialog asks which overlays to include - pose skeleton, segmentation contours, behavior predictions - and clicking **Save** then asks where to write the video. At least one overlay must be selected.
    ...
    - The predictions checkbox is disabled until the current video has been classified. In multi-class projects it is also disabled when the saved predictions were generated for a different behavior list, since their class numbers no longer line up with the project's behaviors; re-classifying the video makes the export available again. The markers match **View→Label Overlay→Predictions**: a colored square beside each animal, showing the same behavior and the same raw or post-processed predictions the timeline is currently showing, whether or not the overlay is switched on in the player. Because an exported video is watched away from JABS, a caption naming the behavior and a legend for the marker colors are burned into the top-left corner.
    - The pose and segmentation overlays are rendered by the same code as **Export Frame**, so a still and a frame of the exported video match. **Export Frame** does not draw the prediction overlay. The export runs in the background with a progress dialog and can be cancelled, which removes the partial file. The last export directory and the overlay selections are remembered.
