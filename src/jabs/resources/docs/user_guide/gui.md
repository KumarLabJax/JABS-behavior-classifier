...
- **File→Export Video with Overlays:** Export a copy of the current video with the JABS overlays drawn on every frame, at the original video resolution. A dialog asks which overlays to include - pose skeleton, segmentation contours, manual labels, behavior predictions - and clicking **Save** then asks where to write the video. At least one overlay must be selected.
    - The segmentation checkbox is disabled when the pose file has no segmentation data to draw - either because it predates version 6, or because it is a version 6 or newer file that was generated without segmentation, which is optional. The tooltip says which applies.
    - Manual labels and behavior predictions are independent checkboxes, so an export can carry either or both. The markers match **View→Label Overlay**: a colored square beside each animal, showing the same behavior and the same raw or post-processed predictions the timeline is currently showing, whether or not the overlay is switched on in the player. With both ticked each animal gets two squares, the manual label on the left and the prediction on the right, as in the player. Because an exported video is watched away from JABS, a caption naming what is drawn and a legend for the marker colors are burned into the top-left corner.
    - The manual labels checkbox is always available for a loaded video, whether or not anything has been labeled yet: an export of a partly labeled video shows a gray marker on the frames that have no label.
    - The predictions checkbox is disabled until the current video has been classified. In multi-class projects it is also disabled when the saved predictions were generated for a different behavior list, since their class numbers no longer line up with the project's behaviors; re-classifying the video makes the export available again. The manual labels stay available in that case, since they are read from the project's current behavior list rather than the saved record.
    - The pose and segmentation overlays are rendered by the same code as **Export Frame**, so a still and a frame of the exported video match. **Export Frame** does not draw the label or prediction markers. The export runs in the background with a progress dialog and can be cancelled, which removes the partial file. The last export directory and the overlay selections are remembered.
...
- **View:** Menu to control various display options.
  ...
  - **View→Label Overlay:** Control the floating display of manual labels, predicted classes, or both at once. "Labels and Predictions" draws two markers beside each animal: the manual label first, then the prediction, so you can compare them frame by frame.
