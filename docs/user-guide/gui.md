...
- **File→Export Video with Pose Overlay:** Export a copy of the current video with the pose overlay drawn on every frame, at the original video resolution. A checkbox on the save dialog controls whether segmentation contours are included; the pose skeleton is always drawn, so unchecking it gives a pose-only video. The checkbox is disabled when the pose file has no segmentation data to draw - either because it predates version 6, or because it is a version 6 or newer file that was generated without segmentation, which is optional. The tooltip says which applies. Overlays are rendered by the same code as **Export Frame**, so a still and a frame of the exported video look identical. The export runs in the background with a progress dialog and can be cancelled, which removes the partial file. The last export directory and the segmentation setting are remembered.
