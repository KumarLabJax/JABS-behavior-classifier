...
model = dict(
    ...
    head=dict(
        ...
        tag_loss=dict(type="AssociativeEmbeddingLoss", loss_weight=1e-3),
        decoder=dict(codec, heatmap_size=codec["input_size"]),
    ),
    train_cfg=dict(max_train_instances=50),
    test_cfg=dict(
...
train_pipeline = [
    ...
    dict(type="PackPoseInputsWithAE"),
]
val_pipeline = [
    dict(type="LoadImage"),
    dict(type="BottomupResize", input_size=input_size, size_factor=64, resize_mode="expand"),
    dict(
